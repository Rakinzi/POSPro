...
class IncomeCategoryRepo {
  ...
  static const String _cacheKey = 'cache:income_categories';
  ...
  Future<List<IncomeCategory>> fetchAllIncomeCategory() async {
    ...
    if (isConnected) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          return maps.map((category) => IncomeCategory.fromJson(category)).toList();
        }
      } catch (_) {
    ...
    return cached.map((item) => IncomeCategory.fromJson(item)).toList();
  }
  ...
  Future<void> addIncomeCategory({
    ...
    if (!isConnected) {
      ...
      await _dbHelper.upsertCachedListItem(_cacheKey, _offlineIncomeCategoryItem(categoryName));
      ref.invalidate(incomeCategoryProvider);
...
Map<String, dynamic> _offlineIncomeCategoryItem(String name) {
  return {
