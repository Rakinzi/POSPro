...
class ExpenseRepo {
  ...
  static const String _cacheKey = 'cache:expenses';
  ...
  Future<List<Expense>> fetchExpense() async {
    ...
    if (isConnected) {
      ...
      if (response.statusCode == 200) {
        ...
        return maps.map((category) => Expense.fromJson(category)).toList();
      }
    }

    final cached = await _dbHelper.getCachedList(_cacheKey);
    return cached.map((item) => Expense.fromJson(item)).toList();
  }
  ...
  Future<void> createExpense({
    ...
    try {
      ...
      if (!isConnected) {
        ...
        await _dbHelper.upsertCachedListItem(_cacheKey, _offlineExpenseItem(
          amount: amount,
          categoryId: expenseCategoryId,
          expenseFor: expanseFor,
          referenceNo: referenceNo,
          expenseDate: expenseDate,
          note: note,
...
Map<String, dynamic> _offlineExpenseItem({
  required num amount,
